...
argocd     arms-prom
$ s2t -n argocd --name argocd-initial-admin-secret # given the namespace and secret, print plain
password: CLG31TzP3S31XX5j
$ s2t -n argocd --name argocd-initial-admin-secret -oenv # print as .env file
password=CLG31TzP3S31XX5j
$ s2t -n argocd --name argocd-initial-admin-secret -oyaml # print as YAML string data
stringData:
    password: CLG31TzP3S31XX5j
$ s2t -n argocd --name argocd-initial-admin-secret -ojsonc # print as compact json (single line)
...
| `--name`            | Name of the resource (secret, or configmap with `--kind`) to fetch live via `kubectl`                                                           | -                                                       |
| `-n`, `--namespace` | Kubernetes namespace (used with `--name`)                                                                                                       | defaults to the kubeconfig's current context if omitted |
...
```bash
s2t --name db-creds -n staging -o env > .env
```
...
```bash
diff <(s2t --name db-creds -n staging) <(s2t --name db-creds -n prod)
```
...
s2t --name app-config -n prod -k configmap        # live fetch
